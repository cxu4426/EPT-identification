...
def get_train_val_test_loaders(batch_size, **kwargs):
    ...
    """
    tr, va, te, _ = get_train_val_test_datasets(**kwargs)
...
def get_train_val_test_datasets(**kwargs):
...
class BugsDataset(Dataset):
    ...
    def __init__(self, partition, augment=True):
        ...
        self.semantic_labels = dict(
            zip(
                self.metadata[self.metadata.partition == self.partition]["numeric_label"],
                self.metadata[self.metadata.partition == self.partition]["semantic_label"],
            )
        )
    ...
    def __resize__(self, img):
        ...
        resized_img = img.resize((config("image_dim"), config("image_dim")))
    ...
    def _load_data(self):
        ...
        for i, row in df.iterrows():
            ...
            image = Image.open(os.path.join(path, row["filename"]))
...
if __name__ == "__main__":
    ...
    tr, va, te, standardizer = get_train_val_test_datasets(augment=True)
